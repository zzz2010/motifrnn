...
public class MotifSentimentModel implements Serializable {
  /**
   * Nx2N+1, where N is the size of the word vectors
   */
  public TwoDimensionalMap<String, String, SimpleMatrix> binaryTransform;  //W
   ...
   */
  public TwoDimensionalMap<String, String, SimpleMatrix> binaryClassification;   //W_out, dont use, when two node from different categories, one additional complexity

  /**
   * CxN+1, where N = size of word vectors, C is the number of classes
   */
  public Map<String, SimpleMatrix> unaryClassification;   //W_out, usually use this, as ICML 2011 paper
   ...
   * corresponding MotifSentimentModel.  Useful for creating a Java version
   * of a model trained in some other manner, such as using the
   * original Matlab code.
   */
  static MotifSentimentModel modelFromMatrices(SimpleMatrix W, SimpleMatrix Wcat, SimpleTensor Wt, Map<String, SimpleMatrix> wordVectors, RNNOptions op) {
    ...
    return new MotifSentimentModel(binaryTransform, binaryTensors, binaryClassification, unaryClassification, wordVectors, op);
  }

  private MotifSentimentModel(TwoDimensionalMap<String, String, SimpleMatrix> binaryTransform,
   ...
   */
  public MotifSentimentModel(RNNOptions op, List<Tree> trainingTrees) {
    this.op = op;
    rand = new Random(op.randomSeed);
  ...
  SimpleMatrix randomTransformBlock() {
    double range = 1.0 / (Math.sqrt(numHid) * 2.0);
  ...
  SimpleMatrix randomClassificationMatrix() {
    ...
    double range = 1.0 / (Math.sqrt(numHid));
...
//    for (String word : rawWordVectors.keySet()) {
  ...
  public SimpleMatrix SingleWordPrediction(String word)
  {
	  Tree node=Tree.valueOf("(1 "+word+")");

	  Iterator<Tree> iter = node.iterator();
	  while(iter.hasNext())
	  {
		  MotifRNNCoreAnnotations.setGoldClass(iter.next(), identity);
	  }

	  SimpleMatrix pred_class = predict(node);
	  return pred_class;
  }
  
  public double SingleWordImportance(String word)
  {
	 
	  SimpleMatrix pred_class = SingleWordPrediction(word);
	  for (int i = 0; i < pred_class.numCols(); i++) {
		  double tmp=pred_class.get(i);
//		  if(tmp>1)
//			  tmp=1;
//		  if(tmp<0)
//			  tmp=0;
  ...
  public Map<String, Float> GetAllWordsImportance()
  {
	  identity= SimpleMatrix.identity(numHid);
	  HashMap<String,Float> importances=new HashMap<String,Float>();
	  for (String word : wordVectors.keySet()) {
		  if(word.contains("UNK"))
			  continue;
		  importances.put(word, (float) SingleWordImportance(word));
	}
	  return Utils.sortByAbsValue(importances);
  }
  ...
  public static MotifSentimentModel loadSerialized(String path) {
    try {
  ...
  {
	MotifSentimentCostAndGradient cag = new MotifSentimentCostAndGradient(this, null,MotifSentimentTraining.neural_function);
	cag.forwardPropagateTree(t);
	return MotifRNNCoreAnnotations.getPredictions(t);
  }
