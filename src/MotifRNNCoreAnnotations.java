...
public class MotifRNNCoreAnnotations {

  private MotifRNNCoreAnnotations() {} // only static members
   ...
   */
  public static class GoldClass implements CoreAnnotation<SimpleMatrix> {
    @Override
	public Class<SimpleMatrix> getType() {
      return SimpleMatrix.class;
    }
  ...
  public static SimpleMatrix getGoldClass(Tree tree) {
    Label label = tree.label();
    if (!(label instanceof CoreLabel)) {
      throw new IllegalArgumentException("CoreLabels required to get the attached gold class");
    }
  ...
  public static void setGoldClass(Tree tree, SimpleMatrix goldClass) {
    Label label = tree.label();
    if (!(label instanceof CoreLabel)) {
      throw new IllegalArgumentException("CoreLabels required to get the attached gold class");
    }
